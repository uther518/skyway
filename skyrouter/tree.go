...
func (n *node) addRoute(path string, handle *skyrewrite.SkyRewrite) {
	fullPath := path
	n.priority++
...
func (n *node) insertChild(numParams uint8, path, fullPath string, handle *skyrewrite.SkyRewrite) {
...
// given path.
func (n *node) getValue(path string, ctx *fasthttp.RequestCtx) (handle *skyrewrite.SkyRewrite, tsr bool, paramCounter int) {
	paramCounter = 0
