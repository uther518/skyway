...
type Router struct {
	...
	// Rewrite and handle request
	OnRequestFunc skyrewrite.RewriteHandler
}
...
// Register rewirte handle
func (r *Router) RewriteHandle(handle skyrewrite.RewriteHandler) {
	r.OnRequestFunc = handle
}

// GET is a shortcut for router.Handle("GET", path, handle)
func (r *Router) GET(path string, handle *skyrewrite.SkyRewrite) {
	r.Handle("GET", path, handle)
}

// HEAD is a shortcut for router.Handle("HEAD", path, handle)
func (r *Router) HEAD(path string, handle *skyrewrite.SkyRewrite) {
	r.Handle("HEAD", path, handle)
}

// OPTIONS is a shortcut for router.Handle("OPTIONS", path, handle)
func (r *Router) OPTIONS(path string, handle *skyrewrite.SkyRewrite) {
	r.Handle("OPTIONS", path, handle)
}

// POST is a shortcut for router.Handle("POST", path, handle)
func (r *Router) POST(path string, handle *skyrewrite.SkyRewrite) {
	r.Handle("POST", path, handle)
}

// PUT is a shortcut for router.Handle("PUT", path, handle)
func (r *Router) PUT(path string, handle *skyrewrite.SkyRewrite) {
	r.Handle("PUT", path, handle)
}

// PATCH is a shortcut for router.Handle("PATCH", path, handle)
func (r *Router) PATCH(path string, handle *skyrewrite.SkyRewrite) {
	r.Handle("PATCH", path, handle)
}

// DELETE is a shortcut for router.Handle("DELETE", path, handle)
func (r *Router) DELETE(path string, handle *skyrewrite.SkyRewrite) {
...
func (r *Router) Handle(method, path string, handle *skyrewrite.SkyRewrite) {
	if path[0] != '/' {
		panic("path must begin with '/' in path '" + path + "'")
	}

	log.Print("freeRouter Run Register.......")

	handle.OriginUri = path
	handle.MakeRegexp()
	path = handle.RouterPath
...
func (r *Router) Lookup(method, path string, ctx *fasthttp.RequestCtx) (*skyrewrite.SkyRewrite, bool, int) {
 ...
 *根据重写规则，重写请求
 */
func (r *Router) RewriteRequest(ctx *fasthttp.RequestCtx, rewriteUri *skyrewrite.SkyRewrite,paramCounter int) {
...
func (r *Router) Handler(ctx *fasthttp.RequestCtx) {
	...
	log.Println("freeRouter Handler start:", path, queryString)
	method := string(ctx.Method())

	if root := r.trees[method]; root != nil {
		if requestHandler, tsr,counter := root.getValue(path, ctx); requestHandler != nil {
			r.RewriteRequest(ctx, requestHandler,counter)
			return
