...
func (n *node) addRoute(path string, rewriteUri *RewriteUri) {
	fullPath := path
	n.priority++
	...
	walk:
		for {
			...
			if i < len(path) {
				...
				n.insertChild(numParams, path, fullPath, rewriteUri)
				return
...
// given path.
func (n *node) getValue(path string, ctx *fasthttp.RequestCtx) (rewriteUri *RewriteUri, tsr bool) {
	...
	for {
		if len(path) > len(n.path) {
			if path[:len(n.path)] == n.path {
				...
				case param:
					...
					if rewriteUri = n.rewriteUri; rewriteUri != nil {
						return
					} else if len(n.children) == 1 {
						// No handle found. Check if a handle for this path + a
						// trailing slash exists for TSR recommendation
						n = n.children[0]
						tsr = (n.path == "/" && n.rewriteUri != nil)
					}
			...
			for i := 0; i < len(n.indices); i++ {
				if n.indices[i] == '/' {
					n = n.children[i]
					tsr = (len(n.path) == 1 && n.rewriteUri != nil) ||
						(n.nType == catchAll && n.children[0].rewriteUri != nil)
					return
				}
			}
		...
		tsr = (path == "/") ||
			(len(n.path) == len(path)+1 && n.path[len(path)] == '/' &&
				path == n.path[:len(n.path)-1] && n.rewriteUri != nil)
		return
	}
}
...
walk: // outer loop for walking the tree
	for len(loPath) >= len(loNPath) && (len(loNPath) == 0 || loPath[1:len(loNPath)] == loNPath[1:]) {
		...
		if path = path[len(n.path):]; len(path) > 0 {
			...
			if !n.wildChild {
				...
				return ciPath, (fixTrailingSlash && path == "/" && n.rewriteUri != nil)
			}
			...
			if fixTrailingSlash {
				for i := 0; i < len(n.indices); i++ {
					if n.indices[i] == '/' {
						n = n.children[i]
						if (len(n.path) == 1 && n.rewriteUri != nil) ||
							(n.nType == catchAll && n.children[0].rewriteUri != nil) {
							return append(ciPath, '/'), true
						}
						return ciPath, false
	...
	if fixTrailingSlash {
		...
		if len(loPath)+1 == len(loNPath) && loNPath[len(loPath)] == '/' &&
			loPath[1:] == loNPath[1:len(loPath)] && n.rewriteUri != nil {
